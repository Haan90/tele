...
bot.onText(/\/start/, (msg) => {
    ...
    const startTime = Date.now();
    const menuText = `
Halo ${sender} Saya Adalah ${nama} Yang Dapat Membantu Anda.
...
bot.on('callback_query', (callbackQuery) => {
  if (callbackQuery.data === 'myprofil') {
    ...
    bot.sendMessage(callbackQuery.from.id, 'Hallo, saya adalah My Profil. Bot Ini Milik @Denzxxoffc');
  }
});
...
bot.on('callback_query', (callbackQuery) => {
    if (callbackQuery.data === 'start') {
        ...
        const menuText = `
Halo ${pushname} Saya Adalah ${nama} Yang Dapat Membantu Anda.
...
bot.onText(/\/ramlist/, (msg) => {
    const chatId = msg.chat.id;
    const sender = msg.from.username;
    const owner = '6819671334'; // Ganti dengan ID pemilik bot 
...
`;
    const keyboard = {
        reply_markup: {
            inline_keyboard: [
                [{ text: '🖥️ Buy Panel', url: 'https://t.me/Denzxxoffc/buy_panel' }, { text: '👤 Buy Admin', url: 'https://t.me/Denzxxoffc/buyadminp & ptpanel' }],
                [{ text: '🇲🇨 Buy Vps', url: 'https://t.me/Denzxxoffc/buyvps' }]
            ]
        }
    };
...
bot.onText(/\/panel/, (msg) => {
    ...
    const owner = '6819671334'; // Ganti dengan ID pemilik bot 
    const text12 = `*Hi @${sender} 👋*
    Untuk Membuat Panel Ketik /ramlist name,id tele
Contoh /1gb Flzz,6819671334
Untuk melihat RamList Ketik /ramlist
`;
    const keyboard = {
        reply_markup: {
            inline_keyboard: [
  ...
  if (!isPremium) {
    bot.sendMessage(chatId, 'Perintah Hanya Untuk Users Premium, Hubungi Admin Saya Untuk Menjadi Users Premium...', {
      reply_markup: {
        inline_keyboard: [
          [
            { text: 'HUBUNGI ADMIN', url: 'https://t.me/Denzxxoffc' }
          ]
        ]
      }
    });
...
==============================
THANKS FOR BUYING AT WANNOFFC😁✌️`
        });
...
                inline_keyboard: [
                    [
                        { text: 'Hubungi Admin', url: 'https://t.me/Flazeeoww' }
                    ]
                ]
            }
        });
